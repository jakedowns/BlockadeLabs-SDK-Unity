...
namespace BlockadeLabsSDK
{
    public class ApiRequests
    {
        ...
        public static async Task<string> CreateImagine(List<GeneratorField> generatorFields, string generator, string apiKey)
        {
            ...
            {
                if(field.param.type == "select")
                {
                    parameters.Add(field.key, field.param.options[field.selectedIndex].value);
                }
                else
                {
                    if (field.value != "")
            ...
            Debug.Log("Requesting w/ parameters: " + parametersJsonString);
